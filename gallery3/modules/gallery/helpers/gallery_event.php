  ...
  static function site_menu($menu, $theme, $item_css_selector) {
    if ($theme->page_subtype != "login") {
      ...
      $menu->append(Menu::factory("link")
                    ...
                    ->label(t("Fashion Challenge"))
                    ->url(url::site("challenge")));
            
      $active = identity::active_user();
      if (!$active->guest) {
          $menu->append($my_show_menu = Menu::factory("submenu")
                          ->id("my_fashion")
                          ->label(t("My Fashion")));
      ...
      if (!empty($item)) {
        ...
        $can_challenge = $item && access::can("challengealbum", $item);
  ...
  static function context_menu($menu, $theme, $item, $thumb_css_selector) {
    ...
    if (access::can("edit", $item)) {
      ...
      default:
        ...
        $dress_search_title = t("Search similar dress");
        break;
      }
      ...
      if ($item->is_photo() && $dress_search_title != NULL) {
        $options_menu
          ->append(
            Menu::factory("link")
            ->id("search_similar_dress")
            ->label(t("Search similar dress"))
            ->css_class("ui-icon-search")
            ->url(url::site("dresssearch?album=1&q=$item->id")));
      }
      ...
      /* [dfw]: remove "Add an album" and "Edit permissions" from the context menu */
      if ($item->is_album()) {
        $options_menu
          ->append(Menu::factory("dialog")
                   ->id("add_item")
