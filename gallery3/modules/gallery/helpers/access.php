...
class access_Core {
  ...
  static function user_can($user, $perm_name, $item) {
    ...
    /* [dfw todo ]: only owner can edit an item if the item is not gallery (id = 1). any better way than comparing the id? */
    if($item->id != 1 && $perm_name == "edit" && $item->owner_id !== identity::active_user()->id) {
        return false;
    }
  ...
  static function can_challenge($user, $item) {
    /* [dfw todo ]: 1. check "accept challenge" in the future. 2. any better way than comparing the id? */
    if (!$item->loaded() || $item->type != 'album' || $item->id == 1) {
      return false;
    }
