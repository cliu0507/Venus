...
class tag_Core {
  ...
  /* [dfw]: add tag by tag id */
  static function add_by_id($item, $tag_id) {
    if (empty($tag_id)) {
      throw new exception("@todo MISSING_TAG_ID");
    }
  ...
  /**
   * [dfw]: Return all the tags.
   *
   * @return ORM_Iterator of Tag_Model
   */
  static function all_tags() {
    return ORM::factory("tag")
      ->find_all();
  }
   ...
   * @return View
   */
  static function cloud($count, $item = NULL) {
    /* [dfw]: cloud only return item associated tags (categories) */
    if(empty($item)) {
  ...
  static function compact() {
    ...
    // extremely rare case.
    /* [dfw]: don't clean 0 count tags since we initially add in tags with 0 count as the category */
