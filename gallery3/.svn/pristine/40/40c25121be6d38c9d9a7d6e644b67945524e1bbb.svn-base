...
abstract class Database_Core {
	...
	// Raw server connection, store as Mysqli object(defined by php) formally, protected
	protected $connection;
	
	//Store $direct_connection as public static, which can be used by static method
	 ...
	 * @return  string
	 * For example, change "items" to "mf_items" 
	 */
	public function quote_table($table, $alias = NULL)
	{
		...
		elseif (strpos(' ', $table) !== FALSE) // strpos: Find the position of the first occurrence of a substring in a string
		{
			// Using format 'user u'
