...
class DressSearch_Controller extends Controller {
  public function index() {
    ...
    item::set_display_context_callback("DressSearch_Controller::get_display_context", $album, $q);
  }
  ...
  static function get_display_context($item, $album, $q) {
    ...
    if ($position > 1) {
      ...
      $previous_item = null;
      list ($count, $result_data) =
        //dresssearchfromdb::search_within_album($q_with_more_terms, $album, 1, $position);
        dresssearchfromdb::search_by_id(intval($q), $album);
    ...
    return array("position" => $position,
                 ...
                 "next_item" => $next_item,
                 "sibling_count" => $count,
                 "siblings_callback" => array("DressSearch_Controller::get_siblings", array($q, $album)),
                 "breadcrumbs" => array(
                   Breadcrumb::instance($root->title, $root->url())->set_first(),
