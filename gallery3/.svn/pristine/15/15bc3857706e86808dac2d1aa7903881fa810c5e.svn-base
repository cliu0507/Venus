...
if (installer::already_installed()) {
  ...
  switch (@$_GET["step"]) {
    /*PHP supports one error control operator: the at sign (@).
     ...
     So just continue the code below
   */
  default:
  case "welcome":
    ...
    break;
    /*
    Please Refer to get_db_info.html.php, it has form using post, but also has value in $_GET, because form pass both post and 
    get request, with "get" in URL, looks like: <form method="post" action="index.php?step=save_db_info"> , so $_GET["step"]=
    save_db_info
     */
  case "save_db_info":
...
//In addition to using functions such as echo and print, you can also end your script,
// and anything beyond the end of the script will be output as normal HTML to the browser. 
// You can also restart your script whenever you want after you've closed the PHP tag. Confused? It's actually pretty simple.
