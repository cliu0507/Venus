...
class Database_Mysqli_Result_Core extends Database_Result {
	...
	public function __construct($result, $sql, $link, $return_objects)
	{
		if (is_object($result))
		{
			// True to return objects(This gallery3 database config is True), false for arrays
	...
	public function as_array($return = FALSE)
	{
		...
		//  If there is only one row in the Dtabase_Mysqli_Result object, directly return
		...
		if (RUNS_MYSQLND)   //define('RUNS_MYSQLND', function_exists('mysqli_fetch_all'));
		...
		if ($this->total_rows > 0)
		{
			...
			while ($row = $this->result->fetch_assoc()) // Fetch a result row as an associative array
			{
	...
	public function current()
	{
		...
		else
		{
			...
			//Returns an associative array of strings representing the fetched row in the result set,
			 //where each key in the array represents the name of one of the result set's columns or NULL
