...
class ORM_Core {
	...
	// Current object, formerly it is protected, change it to public
	public $object  = array();
  ...
  //Create a new variable for direct db manipulations	
	...
	public function __construct($id = NULL)
	{
		...
		//$this->object_name = item/comment/tag/rating         $this is item_Model object
		$this->object_plural = inflector::plural($this->object_name); 
		//$this->object_plural = items/comments/tags/ratings   $this is item_Model/comment_Model object
		...
		if (is_object($id))
		{			
			//If $id is an object, this object must be the an object of a query result(stored by object)
	...
	public function __initialize()
	{
		...
		if (empty($this->table_name))
		{
			...
			$this->table_name = $this->object_name; //$this->table_name = item

			if ($this->table_names_plural === TRUE)
			{
				// Make the table name plural
				$this->table_name = inflector::plural($this->table_name);//$this->table_name = items
			}
		}
	...
	public function as_array()
	{
		...
		{
			//$this->object stores query result
	...
	public function reload_columns($force = FALSE)
	{
		if ($force === TRUE OR empty($this->table_columns))
		{
			if (isset(ORM::$column_cache[$this->object_name]))
			{
				...
				$this->table_columns = ORM::$column_cache[$this->object_name];//ORM::$column_cache is static
			}
			else
			{
				// Load table columns
		...
		$this->table_columns = $this->list_fields() = result 
		= array(
			 ...
			 "id"         => array(
			            ...
			            "Default" => Null
			            ...
			            "sequenced" => auto_increment
			            ...
			              ),
			
			 "album_cover_item_id" => array(
			            "Field" => album_cover_item_id 
			            "Type"  => int(9)
			            "Null"  => NO
			            "Key"   => Pri
			            "Default" => Null
	...
	public function list_fields()
	{
		...
		// go to Database_Mysqli object ->list_fields() Actually, list_fields is in his superclass Database_Mysql.php
	}
	...
	protected function load_result($array = FALSE, $ignore_changed = FALSE)
	{
		...
		// Load the resultexecute sql query!!!
		...
		if ($result->count() === 1)
		{// if result is only one row
			// Load object values
			//$result->as_array()->current() is an array, every key is every column's name
